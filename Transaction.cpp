...
void return_transaction(char find_idOfReader[],char book[100][8][100],char transaction[100][6][100],char actual_return_date[],char ISBNs[]) {
	...
	//split transaction[i][4] (1d char array) into ISBNs_borrow (2d char array) which contains all ISBN have been borrowed
	int count=0;
	int row = 0, col = 0;
	...
	int total_fine_amount=charArrayToNumber(transaction[i][5]),price;
